...
// generateCode redirects to showSetup via PRG.
// showSetup generates a fresh code on every GET, so the redirect is sufficient.
func (ctr *slackSetupController) generateCode(c web.APIContext) error {
	user := web.GetDBLoggedUserFromSession(c)
	if user == nil {
		return &echo.HTTPError{Code: http.StatusUnauthorized, Message: "not logged in"}
	}
	_ = user // auth check only; code is generated by showSetup on redirect
