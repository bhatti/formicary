...
// showSetup redirects to the user profile Slack tab (the setup page is now
// embedded there). Keeps the old URL working for bookmarks.
func (ctr *slackSetupController) showSetup(c web.APIContext) error {
	...
	return c.Redirect(http.StatusSeeOther, "/dashboard/users/"+user.ID+"#slack")
}

// generateCode redirects to the user profile Slack tab.
